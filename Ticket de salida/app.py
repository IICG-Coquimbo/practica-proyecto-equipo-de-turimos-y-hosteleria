...
st.markdown("**Camila Rojas** | Analisis de alojamientos por segmento")
st.markdown("---")
...
df["segmento"] = df["prediction"].map(etiquetas).fillna("Otro")
...
# PESTANA 1
with tab_est:
    st.header("Participacion de Mercado por Segmento")
    st.caption("Frecuencia: Mensual | Objetivo: Identificar que segmento domina el mercado")

    total = len(df)
    df_est = df["segmento"].value_counts().reset_index()
    df_est.columns = ["segmento", "Cantidad"]
    df_est["Participacion"] = (df_est["Cantidad"] / total) * 100

    col1, col2 = st.columns([1, 2])
    with col1:
        st.metric("Total Alojamientos", total)
        st.metric("Segmento Lider", df_est["segmento"].iloc[0],
                  delta=f"{df_est['Participacion'].iloc[0]:.1f}% del mercado")
        st.dataframe(df_est, hide_index=True)

    with col2:
        fig, ax = plt.subplots(figsize=(8, 4.5))
        colors = {"Economico": "#4CAF50", "Intermedio": "#2196F3", "Premium": "#9C27B0", "Otro": "#gray"}
        sns.barplot(x="Participacion", y="segmento", data=df_est,
                    hue="segmento", palette="Blues_r", legend=False, ax=ax)
        for p in ax.patches:
        ...
        ax.set_title("Participacion por Segmento de Alojamiento")
        sns.despine(left=True)
        st.pyplot(fig)

# PESTANA 2
with tab_tac:
    st.header("Bandas de Precios por Segmento")
    st.caption("Frecuencia: Semanal | Objetivo: Comparar precios entre segmentos")

    segmentos = st.multiselect(
        "Filtrar Segmentos:",
        options=df["segmento"].unique(),
        default=list(df["segmento"].unique())
    )

    df_filtrado = df[df["segmento"].isin(segmentos)]
    df_tac = df_filtrado.groupby("segmento")["precio_noche"].agg(["min", "mean", "max"]).reset_index().dropna()
    df_tac = df_tac.sort_values("mean")

    col1, col2 = st.columns([1, 2])
    with col1:
        st.metric("Precio Promedio General", f"${df_filtrado['precio_noche'].mean():,.0f} CLP")
        st.metric("Segmento mas caro", df_tac.iloc[-1]["segmento"],
                  delta=f"${df_tac.iloc[-1]['mean']:,.0f} CLP promedio")
        st.metric("Segmento mas economico", df_tac.iloc[0]["segmento"],
                  delta=f"${df_tac.iloc[0]['mean']:,.0f} CLP promedio")

    with col2:
        fig, ax = plt.subplots(figsize=(8, 5))
        ax.vlines(x=df_tac["segmento"], ymin=df_tac["min"], ymax=df_tac["max"],
                  colors="#B0BEC5", alpha=0.7, linewidth=3)
        ax.scatter(df_tac["segmento"], df_tac["mean"], color="#1A237E", s=120, zorder=3, label="Promedio")
        ax.scatter(df_tac["segmento"], df_tac["min"], color="#2E7D32", marker="^", s=80, zorder=3, label="Minimo")
        ax.scatter(df_tac["segmento"], df_tac["max"], color="#C62828", marker="v", s=80, zorder=3, label="Maximo")
        ax.set_ylabel("Precio por Noche (CLP)")
        ax.set_xlabel("Segmento")
        ax.set_title("Bandas de Precios por Segmento")
        ax.legend()
        sns.despine(left=True)
        plt.tight_layout()
        st.pyplot(fig)
...
# PESTANA 3
with tab_op:
    st.header("Alertas: Alojamientos Caros con Baja Puntuacion")
    ...
    umbral_puntuacion = st.slider("Umbral critico de puntuacion:", 0.0, 10.0, 7.0, 0.1)
    umbral_precio = st.slider(
        "Umbral de precio alto (CLP):",
        int(df["precio_noche"].quantile(0.25)),
        int(df["precio_noche"].quantile(0.90)),
        int(precio_promedio),
        5000
    )

    df_op = df[["precio_noche", "puntuacion", "estrellas", "segmento"]].dropna()
    zona_peligro = df_op[
    ...
    with col2:
        st.metric("Total alertas", len(zona_peligro))
    ...
    if not zona_peligro.empty:
        ...
        st.dataframe(zona_peligro.sort_values("precio_noche", ascending=False), hide_index=True)
